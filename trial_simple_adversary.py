...
agents_dict = {"adversary_0": adversary0_PPO, "agent_0": agent0_PPO, "agent_1": agent1_PPO}
un_agents_dict = {"adversary_0": un_adversary0_PPO, "agent_0": un_agent0_PPO, "agent_1": un_agent1_PPO}
...
def evaluation(agents_dict, episodes):
    ...
    for e in range(episodes):
        if (e%100 == 0):
            print("Episode:", e)
        adv0r[e], agent0r[e], agent1r[e] = evaluate_episode(agents_dict)
...
def evaluate_episode(agents_dict):
    ...
    env.reset()
    adv0r = np.zeros(cycles); agent0r = np.zeros(cycles); agent1r = np.zeros(cycles)
    i = 0
...
def plot_hist_returns(rews_before, rews_after):
    ...
    ax[1,0].hist(returns_ag0b, bins=n_bins, range=[-30., 100.], label='ag0')
    ax[1,1].hist(returns_ag0a, bins=n_bins, range=[-30., 100.], label='ag0')
    ax[2,0].hist(returns_ag1b, bins=n_bins, range=[-30., 100.], label='ag1')
    ax[2,1].hist(returns_ag1a, bins=n_bins, range=[-30., 100.], label='ag1')
...
while time_step <= max_training_timesteps:
    ...
    i_internal_loop = 0
    adversary0_PPO.tmp_return = 0; agent0_PPO.tmp_return = 0; agent1_PPO.tmp_return = 0
    ...
    for id_agent in env.agent_iter(max_ep_len):
        ...
        if time_step % print_freq == 0:
            ...
            print("Average Reward : adv0={}, agent0={}, angent1={}".format(print_avg_reward[0], print_avg_reward[1], print_avg_reward[2]))
...
print("mov_avg_adv0=",mov_avg_adv0)
print("mov_avg_agent0=",mov_avg_agent0)
print("mov_avg_agent1=",mov_avg_agent1)

fig, (ax1, ax2, ax3) = plt.subplots(3)
fig.suptitle("Train Returns")
ax1.plot(np.linspace(0, len(adversary0_PPO.train_returns), len(adversary0_PPO.train_returns)), adversary0_PPO.train_returns)
ax1.plot(np.linspace(0, len(adversary0_PPO.train_returns), len(mov_avg_adv0)), mov_avg_adv0)
ax2.plot(np.linspace(0, len(agent0_PPO.train_returns), len(agent0_PPO.train_returns)), agent0_PPO.train_returns)
ax2.plot(np.linspace(0, len(agent0_PPO.train_returns), len(mov_avg_agent0)), mov_avg_agent0)
ax3.plot(np.linspace(0, len(agent1_PPO.train_returns), len(agent1_PPO.train_returns)), agent1_PPO.train_returns)
ax3.plot(np.linspace(0, len(agent1_PPO.train_returns), len(mov_avg_agent1)), mov_avg_agent1)
...
df = pd.DataFrame(columns=['episode', 't','adv0_ret', 'agent0_ret', 'agent1_ret'])
line = 0
