...
def interaction_loop(config, parallel_env, active_agents, active_agents_idxs, social_norm, _eval=False, mf_input=None):
    ...
    for i in range(config.num_game_iterations):
        ...
        for idx_agent, agent in active_agents.items():
            ...
            #print("st=", st)
        #print("rep_next_states=", rep_next_states)
        ...
        rep_actions = {}
        for idx_agent, agent in active_agents.items():
            rep_actions[idx_agent] = agent.select_reputation_assignment(rep_states[idx_agent])
...
def train_q_learning_reputation_assignment(args):
    ...
    repo_name = "EPGG_"+ str(args.n_agents) + "agents_" + \
        ...
        "_rep" + str(args.reputation_enabled) + "_reputation_assignment"
