...
def objective(args, repo_name, trial=None):
    ...
    for epoch in range(config.n_episodes):
        ...
        for ag_idx, agent in agents.items():
            ...
            if (agent.is_dummy == False):
                if ag_idx not in prob:
                    prob[ag_idx] = torch.zeros(len(config.mult_fact), 2) # mult fact, poss rep, poss actions
        ...
        if (config.wandb_mode == "online" and float(epoch)%10. == 0.):
            ...
            dff_prob01 = {"avg_prob["+str(mf)+",1]": avg_distrib[idx_m,0] for idx_m, mf in enumerate(config.mult_fact) }
            dff = {
                "epoch": epoch,
                "avg_rep": avg_rep,
            ...
            if (config.non_dummy_idxs != []): 
                dff = {**dff, **dff_coop_per_mf, **dff_prob00, **dff_prob01}
            wandb.log(dff,
...
def train_reinforce_no_reputation(args):
