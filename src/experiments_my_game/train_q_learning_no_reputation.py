            ...
            for ag_idx, agent in active_agents.items():
                if (agent.is_dummy == False):
                    ...
                    if (len(config.mult_fact) == 1):
                        df_Q1 = dict((ag_idx+"Q["+str(mf)+",0]", agent.Q[0] ) for _, mf in enumerate(config.mult_fact))
                        df_Q2 = dict((ag_idx+"Q["+str(mf)+",1]", agent.Q[1] ) for _, mf in enumerate(config.mult_fact))
                    else:  
                    ...
                    df_agent = {**{
                        ...
                        'epoch': epoch}, 
                        **df_avg_coop, **df_avg_rew, **df_Q1, **df_Q2
                        }
                else:
...
def train_q_learning_no_reputation(args):
