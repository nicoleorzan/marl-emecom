...
from src.algos.Agent_model import Agent
import numpy as np
import optuna
...
from utils_train_reinforce_comm import eval_ps, partner_selection
...
def setup_training_hyperparams(trial, args):
    ...
    all_params = dict(
        ...
        partner_selection = args.partner_selection, # True/False
        punishment = args.punishment, # True/False
...
def define_agents(config):
    ...
    for idx in range(config.n_agents):
        agents['agent_'+str(idx)] = Agent(config, idx)
    return agents
...
def objective(trial, args, repo_name):
    ...
    max_values = find_max_min(config, 4, n_playing_agents=2)
    ...
    for epoch in range(config.n_epochs): 
        ...
        for _ in range(config.batch_size):
            ...
            while not done:
                ...
                for ag_idx, ag in playing_agents.items():
                    actions[ag_idx] = ag.select_action(m_val=mf.numpy()[0]) # m value is given only to compute metrics
                print("actions=", actions)

                observations, rewards, done, _ = parallel_env.step(actions)

                for ag_idx, agent in playing_agents.items():
                    agent.set_reward(rewards[ag_idx], max_values[float(parallel_env.current_multiplier[0])])
        ...
        for m in config.mult_fact:
            print("\nm=", m)
            act_eval, mex_distrib, _, rewards_eval = eval_ps(config, parallel_env, agents, m, device, False)
        ...
        """print("[agent.return_episode_old_norm for _, agent in agents.items()]=", [agent.return_episode_old_norm for _, agent in agents.items()])
        ...
        rew_values = [(np.sum([rewards_eval_m[m_val][ag_idx] for m_val in config.mult_fact if ag_idx in rewards_eval_m[m_val]])) for ag_idx, _ in agents.items()]
...
def training_function(args):
    ...
    repo_name = str(args.n_agents) + "agents_" + "comm" + str(args.communicating_agents) + \
        ...
        "_mfact" + str(args.mult_fact) + str(args.partner_selection) + "_PartnerSelection"
