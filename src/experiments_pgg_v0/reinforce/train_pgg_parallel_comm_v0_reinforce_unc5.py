...
wandb.init(project="reinforce_pgg_v0_comm_unc5", entity="nicoleorzan", config=hyperparameter_defaults, mode=hyperparameter_defaults["wandb_mode"], sync_tensorboard=True)
config = wandb.config
...
def train(config):
    ...
    if (config.save_data == True):
        print("Saving data")
        df.to_csv(path+'data_comm_unc5'+time.strftime("%Y%m%d-%H%M%S")+'.csv')
